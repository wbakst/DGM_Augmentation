...
class Generator(torch.nn.Module):
    def __init__(self, name='generator', dim_z=64, num_channels=1):
        super().__init__()
        self.name = name
        self.dim_z = dim_z
...
class SpectralDiscriminator(torch.nn.Module):
    def __init__(self, num_channels=1):
        super().__init__()
        self.net = torch.nn.Sequential(
            spectral_norm(torch.nn.Conv2d(1, 32, kernel_size=4, stride=2, padding=1)),
            torch.nn.LeakyReLU(0.1, inplace=True),

            spectral_norm(torch.nn.Conv2d(32, 64, kernel_size=4, stride=2, padding=1)),
            torch.nn.LeakyReLU(0.1, inplace=True),

            Reshape(64 * 7 * 7),
            spectral_norm(torch.nn.Linear(64 * 7 * 7, 512)),
            torch.nn.LeakyReLU(0.1, inplace=True),

            spectral_norm(torch.nn.Linear(512, 1)),
            Reshape()
        )
...
class ConditionalGenerator(torch.nn.Module):
    def __init__(self, name='generator', dim_z=64, num_channels=1, num_classes=10):
        super().__init__()
        self.name = name
        self.dim_z = dim_z
...
class ConditionalSpectralDiscriminator(torch.nn.Module):
    def __init__(self, num_classes=10):
        super().__init__()
        self.num_classes = num_classes
        self.net = torch.nn.Sequential(
            ...
            spectral_norm(torch.nn.Linear(64 * 7 * 7, 512)),
            torch.nn.LeakyReLU(0.1),
            spectral_norm(torch.nn.Linear(512, self.num_classes))
        )
